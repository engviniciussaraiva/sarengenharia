...
function normalizeState(raw){
  ...
  next.meta={...defaults.meta,...(raw?.meta||{})};next.basin={...defaults.basin,...(raw?.basin||{})};next.tanks=(raw?.tanks||[]).map(item=>tank(item));next.distances={...(raw?.distances||{})};
...
function loadSystem(){
  try{
    const current=localStorage.getItem(SYSTEM_STORAGE);
    if(current)return normalizeSystem(JSON.parse(current));
    const previous=localStorage.getItem('sar.parque.tanques.multibacia.v30')||localStorage.getItem('sar.parque.tanques.multibacia.v29');
    if(previous)return normalizeSystem(JSON.parse(previous));
  }catch{}
  return normalizeSystem(null);
}
...
function neighbor(fire,other){
  ...
  if(state.distances?.[pairKey(fire,other)]===undefined)return false;
  if(fire.orientation==='horizontal') return false;
  return shellDistance(fire,other)<Math.max(1.5*num(fire.diameter),15);
}
...
function scenario(fire){
  ...
  const coolingVolume=coolingFlow*num(fire.coolingTime)/1000;
...
function basinCalc(){
  ...
  return {area,calculatedArea,occupied,usefulArea,largest,bases,combat,required,hydraulicHeight,finalHeight};
}
function bindStatic(){
...
function renderDistanceInputMatrix(){
  ...
  $$('[data-distance-key]').forEach(el=>prepareNumericInput(el,value=>{state.distances[el.dataset.distanceKey]=value;save();renderDistances();renderResults();validate()}));
}
function renderDistances(){
  const fire=state.tanks.find(t=>t.id===$('#fireTank').value)||state.tanks[0];if(!fire)return;
  renderDistanceInputMatrix();
  const ns=neighbors(fire),others=state.tanks.filter(t=>t.id!==fire.id),diameterReference=1.5*num(fire.diameter),isolationLimit=fire.orientation==='vertical'?Math.max(diameterReference,15):15;
  $('#neighborSummary').textContent=`${ns.length} vizinho(s) de ${Math.max(0,state.tanks.length-1)}`;
  $('#distanceTable').innerHTML='<thead><tr><th>Tanque analisado</th><th>Costado–costado informado</th><th>Referência pelo diâmetro</th><th>Limite adotado</th><th>Vizinho?</th><th>Justificativa</th></tr></thead><tbody>'+state.tanks.filter(t=>t.id!==fire.id).map(t=>{const diameterReference=1.5*num(fire.diameter),limit=fire.orientation==='vertical'?Math.max(diameterReference,15):15;const key=pairKey(fire,t),has=state.distances?.[key]!==undefined,yes=neighbor(fire,t);return `<tr><td>${t.tag}</td><td>${has?fmt(shellDistance(fire,t))+' m':'Não informado'}</td><td><span class="diameter-reference">1,5 × ${fmt(fire.diameter)} m = <b>${fmt(diameterReference)} m</b></span></td><td><b>${fmt(limit)} m</b></td><td class="${fire.orientation==='horizontal'||!has?'muted':yes?'ok':'bad'}">${fire.orientation==='horizontal'?'PENDENTE':!has?'INFORMAR':yes?'SIM':'NÃO'}</td><td>${fire.orientation==='horizontal'?'Tanque horizontal exige a distância direcional entre o dique e o costado adjacente.':!has?'Preencha a matriz de distâncias.':`distância costado ${yes?'menor':'maior/igual'} ao limite`}</td></tr>`}).join('')+'</tbody>';
  $('#matrixTable').innerHTML='<thead><tr><th>Em chamas ↓ / Analisado →</th>'+state.tanks.map(t=>`<th>${t.tag}</th>`).join('')+'<th>Total</th></tr></thead><tbody>'+state.tanks.map(f=>`<tr><th>${f.tag}</th>${state.tanks.map(t=>`<td class="${f.id===t.id?'muted':neighbor(f,t)?'ok':'bad'}">${f.id===t.id?'—':neighbor(f,t)?'SIM':'NÃO'}</td>`).join('')}<td><b>${neighbors(f).length}</b></td></tr>`).join('')+'</tbody>';
}
function renderProtection(){
  const table=$('#primaryFoamTable');if(!table)return;
  table.innerHTML=`<thead><tr>
    <th>Referência</th><th>Tipo de aplicação</th><th>Taxa de aplicação<br><small>(L/min/m²)</small></th>
    <th>Majoração vento<br><small>(20%)</small></th><th>Taxa de aplicação majorada<br><small>(L/min/m²)</small></th>
    <th>Tempo de aplicação<br><small>(min)</small></th><th>Dosagem LGE<br><small>(%)</small></th>
    <th>Vazão de solução mínima<br><small>(L/min)</small></th><th>Quantidade LGE combate<br><small>(litros)</small></th>
  ...
  $('#samePrimaryFoamPrompt').hidden=state.tanks.length<2||(!num(state.tanks[0]?.foamRate)&&!num(state.tanks[0]?.foamTime));
  renderSecondaryFoam();
}
...
function renderScenarioDetail(){
  ...
  host.innerHTML=scenarios().map((s,index)=>{
    ...
    return `<details class="step-card cooling-scenario" ${index===0?'open':''}>
      <summary><span>${index+1}</span><div><b>Cenário — ${fire.tag} em chamas</b><small>${s.ns.length} tanque(s) vizinho(s) considerado(s)</small></div></summary>
      <div class="step-body">
        <div class="cooling-controls">
          <label>Tipo de resfriamento<select class="normative" data-cooling-id="${fire.id}" data-cooling-key="coolingMethod"><option value="monitor" ${fire.coolingMethod==='monitor'?'selected':''}>Canhão-monitor</option><option value="linha" ${fire.coolingMethod==='linha'?'selected':''}>Linha manual</option><option value="aspersao" ${fire.coolingMethod==='aspersao'?'selected':''}>Aspersão</option></select></label>
          <label>Taxa — tanque em chamas (L/min/m²)<input class="normative" type="text" inputmode="decimal" data-numeric="true" value="${inputNumberValue(fire.coolingOwnRate)}" data-cooling-id="${fire.id}" data-cooling-key="coolingOwnRate"></label>
        </div>
        <p class="cooling-area-note"><b>Regra corrigida:</b> ${fire.tag} em chamas recebe resfriamento somente no costado. Nos tanques vizinhos, teto fixo recebe teto + costado e teto flutuante recebe somente costado. Taxas temporariamente editáveis até a conexão com o banco normativo.</p>
        <div class="cooling-layout">
          <div class="table-wrap cooling-table-wrap"><table class="cooling-table"><thead><tr><th>Parâmetro</th><th>${fire.tag} em chamas</th>${others.map(t=>`<th>${t.tag}</th>`).join('')}</tr></thead><tbody>
            ...
            <tr><th>Área de aplicação (m²)</th><td>${fmt(fireCoolingArea(fire))}<small class="cell-hint">${roofRule}</small></td>${others.map(t=>`<td>${fmt(neighborCoolingArea(t))}<small class="cell-hint">${t.roofType==='fixo'?'teto + costado':'somente costado'}</small></td>`).join('')}</tr>
            <tr><th>Taxa de aplicação (L/min/m²)</th><td>${fmt(fire.coolingOwnRate)}</td>${others.map(t=>`<td>${neighbor(fire,t)?`<input class="normative cooling-rate-input" type="text" inputmode="decimal" data-numeric="true" value="${inputNumberValue(neighborCoolingRate(fire,t))}" data-cooling-neighbor-fire="${fire.id}" data-cooling-neighbor-tank="${t.id}">`:'—'}</td>`).join('')}</tr>
            <tr><th>Vazão de resfriamento (L/min)</th><td>${fmt(s.ownCooling)}</td>${others.map(t=>`<td>${neighbor(fire,t)?fmt(neighborCoolingArea(t)*neighborCoolingRate(fire,t)):'—'}</td>`).join('')}</tr>
          ...
          <aside class="cooling-summary"><div class="cooling-summary-title">Resumo — ${fire.tag}</div>
            ...
            <div class="summary-row"><span>Tempo de aplicação</span><label><input class="normative" type="text" inputmode="decimal" data-numeric="true" value="${inputNumberValue(time)}" data-cooling-id="${fire.id}" data-cooling-key="coolingTime"><small class="cell-hint">critério do banco</small></label></div>
